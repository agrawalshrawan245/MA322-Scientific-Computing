...
ans = gauss(f,x,y,bc_xa,bc_xb,bc_ya,bc_yb);
...
function output = gauss(f,x,y,bc_xa,bc_xb,bc_ya,bc_yb)
    n=length(x)-1;
    h=x(2)-x(1);
    ...
    while df>tol
        ...
        for k=2:n
            for j=2:n
                u=0.25*(output(k-1,j)+output(k,j-1)+output(k,j+1)+output(k+1,j)-h*h*f(x(k),y(j)));
                df=max(df,abs(u-output(k,j)));
                output(k,j)=u;
            end
        end
    end
